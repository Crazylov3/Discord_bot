...
def weather_get_info(json):
    ...
    for d in days.keys():
        ...
        dic["temp"] = round(np.mean([list_day[i]['main']['temp'] for i in days[d]]) - 273)
        dic["temp_min"] = round(min([list_day[i]['main']['temp_min'] for i in days[d]]) - 273)
        dic["temp_max"] = round(max([list_day[i]['main']['temp_max'] for i in days[d]]) - 273)
        dic["humidity"] = round(np.mean([list_day[i]['main']['humidity'] for i in days[d]]), 2)
        dic["weather_des"] = mode([list_day[i]["weather"][0]["description"] for i in days[d]])
        dic["wind_speed"] = round(np.mean([list_day[i]['wind']['speed'] for i in days[d]]), 2)
        dic["wind_gust"] = max([list_day[i]['wind']['gust'] for i in days[d]])
        dic["city_name"] = json['city']['name']
        info.append(dic)
    return info[:4]
        ...
        async with ctx.channel.typing():
            ...
            for name in city_name:
                if name in file.keys():
                    city_code = file[name]
                    url = f"https://api.openweathermap.org/data/2.5/forecast?id={city_code}&APPID=cd345a07012bfa798d77ab7e594ad2b2"
